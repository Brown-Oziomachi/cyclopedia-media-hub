...
const SportsPage = () => {
    ...
    const currentSport = sportsCategories.find((s) => s.id === sportParam) || sportsCategories[0];
        ...
        const fetchSportsVideos = async () => {
            ...
            try {
                ...
                const response = await fetch(
                    `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=10&q=${encodeURIComponent(searchQuery)}&type=video&order=date&videoDuration=medium&key=${YOUTUBE_API_KEY}`
                );
    ...
    useEffect(() => {
        if (sportParam === "all") {
            ...
            const filtered = posts.filter(
                (post) =>
                    post.subcategory?.toLowerCase() === sportParam.toLowerCase()
            );
    ...
    return (
        <main className="w-full relative">
            ...
            <section className="relative w-full h-[500px] lg:mt-30 mt-10 flex items-center justify-center text-center overflow-hidden">
                <div
                    className={`absolute inset-0 bg-cover bg-center transition-opacity duration-1000 ${showVideo && spaceVideos.length > 0 ? "opacity-0" : "opacity-100"
                        }`}
                    style={{
                ...
                {showVideo && spaceVideos.length > 0 && spaceVideos[currentVideoIndex] && (
            ...
            {/* Sidebar Menu */}
            <div
                className={`fixed top-0 right-0 h-full w-full md:w-96 bg-gradient-to-b from-gray-900 to-black text-white shadow-2xl z-50 transform transition-transform duration-300 overflow-y-auto ${sidebarOpen ? "translate-x-0" : "translate-x-full"
                    }`}
            >
                <div className="p-6">
                    ...
                    <div className="space-y-3">
                        {sportsCategories.map((sport) => {
                            const sportPosts = sport.id === "all"
                                ? posts
                                : posts.filter((p) => p.subcategory?.toLowerCase() === sport.id.toLowerCase());
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div
                                        className={`px-3 py-1 rounded-full text-xs font-bold ${isActive ? "bg-white/20" : "bg-gray-700"
                                            }`}
                                    >
                                        {sportPosts.length}
                                    </div>
            ...
            <div id="sports-content" className="max-w-7xl mx-auto py-10 px-4">
                ...
                <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-gray-500 text-sm">{filteredPosts.length} articles available</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="mb-16 bg-gradient-to-br from-indigo-900/20 to-purple-900/20 rounded-2xl p-8 border border-indigo-500/30">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="text-2xl font-bold">Latest {currentSport.name} Videos</h3>
                                <p className="text-sm text-gray-500">Click any video to play at the top</p>
                            </div>
                        </div>
                    </div>
                    ...
                    {loadingVideos ? (
                        <div className="text-center py-10">
                            ...
                            <p className="mt-4 text-gray-500">Loading {currentSport.name.toLowerCase()} videos...</p>
                        </div>
                        ...
                        <div className="text-center py-10 bg-yellow-900/20 rounded-xl border border-yellow-500/30 p-6">
                            ...
                            <p className="text-yellow-400 font-semibold mb-2">No videos available</p>
                            <p className="text-gray-400 text-sm">Check your YouTube API key configuration</p>
                            <p className="text-gray-500 text-xs mt-2">Open browser console (F12) for details</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {spaceVideos.map((video, index) => (
                                <button
                                    ...
                                    className={`group relative rounded-xl overflow-hidden bg-gray-900 hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 text-left ${currentVideoIndex === index ? 'ring-4 ring-yellow-400' : ''
                                        }`}
                                >
                                    <div className="relative h-48 overflow-hidden">
                                        ...
                                        <div className={`absolute top-3 right-3 p-2 rounded-full ${currentVideoIndex === index ? 'bg-yellow-400' : 'bg-indigo-600'
                                            } text-white`}>
                                            <Play size={16} />
                                        </div>
                ...
                {filteredPosts[0] && (
                    <div className="mb-12">
                        <Link href={`/news/${createFullSlug(filteredPosts[0].title, filteredPosts[0].id)}`}>
                            <div className="relative grid lg:grid-cols-2 gap-6 rounded-2xl overflow-hidden shadow-2xl hover:shadow-3xl transition-all duration-300 bg-white dark:bg-gray-800">
                                {filteredPosts[0].imageUrl && (
                                    <div className="relative h-64 lg:h-96 overflow-hidden">
                                        ...
                                        />
                                        <div className={`absolute top-4 left-4 bg-gradient-to-r ${currentSport.color} text-white text-sm font-bold px-4 py-2 rounded-full shadow-lg`}>
                                ...
                                <div className="p-8 flex flex-col justify-center">
                                    ...
                                    <div className="flex items-center gap-4 text-sm text-gray-500">
                                        <span>{filteredPosts[0].createdAt?.toDate().toDateString()}</span>
                                        <span>•</span>
                ...
                {loading ? (
                    <div className="text-center py-20">
                        ...
                        <p className="mt-6 text-gray-600 dark:text-gray-400 text-lg">Loading {currentSport.name.toLowerCase()} news...</p>
                    </div>
                    ...
                    <div className="text-center py-20 bg-gray-100 dark:bg-gray-800 rounded-2xl">
                        ...
                        <p className="text-xl text-gray-500 dark:text-gray-400 mb-2">
                            No posts found for {currentSport.name}
                        </p>
                        <p className="text-sm text-gray-400 mb-6">Check back soon for updates!</p>
                        <button
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
                            {filteredPosts.slice(1).map((post) => (
                                <Link
                                    ...
                                    className="group relative rounded-xl shadow-lg hover:shadow-2xl transition-all overflow-hidden bg-white dark:bg-gray-800 hover:-translate-y-2 duration-300"
                                >
                                    {post.imageUrl && (
                                        <div className="relative h-48 overflow-hidden">
                                            <img
                                                ...
                                                className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                                            />
                                            <div className={`absolute top-3 left-3 bg-gradient-to-r ${currentSport.color} text-white text-xs font-semibold px-3 py-1 rounded-full shadow-lg`}>
                                                {post.subcategory || "Sports"}
                                            </div>
                                        </div>
                                    )}
                                    <div className="p-5">
                                        <h2 className="text-lg font-bold mb-2 group-hover:text-red-600 transition-colors line-clamp-2 uppercase">
                                            {post.title}
                                        </h2>
                                        ...
                                        <div className="flex items-center justify-between text-xs text-gray-500">
                                            ...
                                            <span className="text-red-600 font-semibold group-hover:translate-x-1 transition-transform">→</span>
                                        </div>
                                    </div>
                                </Link>
                            ))}
