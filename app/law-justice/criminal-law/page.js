...
const CriminalLawPage = () => {
  ...
  useEffect(() => {
    const fetchCriminalLawPosts = async () => {
      try {
        ...
        console.log("Fetched Criminal Law posts:", data);
        setPosts(data);
      } catch (error) {
        console.error("Error fetching criminal law posts:", error);
      } finally {
        setLoading(false);
      }
    };

    fetchCriminalLawPosts();
  }, []);
  ...
  return (
    <main className="w-full">
      <section className="relative w-full h-[500px] flex items-center justify-center text-center bg-gradient-to-br from-slate-900 via-gray-900 to-black overflow-hidden">
        {/* Background Overlays */}
        <div className="absolute inset-0 bg-black/40"></div>
        <div className="absolute w-[400px] h-[400px] bg-red-900/30 blur-3xl rounded-full top-10 left-20 animate-pulse"></div>
        <div className="absolute w-[300px] h-[300px] bg-orange-800/30 blur-3xl rounded-full bottom-10 right-20 animate-pulse delay-1000"></div>
        ...
        <div className="relative z-10 px-4 lg:mt-30">
          ...
          <h1 className="text-white font-extrabold text-5xl md:text-6xl lg:text-7xl uppercase mb-4 drop-shadow-lg">
            Defending Justice & Rights
          </h1>
          <p className="text-gray-100 max-w-2xl mx-auto text-sm md:text-lg leading-relaxed">
            Expert insights on criminal defense, prosecutions, sentencing, and your constitutional rights in the justice system.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 lg:py-40 py-20">
                ...
                <div className="relative rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 h-full">
                  <div className="flex gap-3 p-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-base font-bold line-clamp-3 hover:text-red-600 transition-colors">
                        {post.title}
                      </h3>
                    </div>
                  </div>
        ...
        <div className="mx-auto text-center mt-16 mb-16 bg-gradient-to-r from-red-700 to-orange-700 text-white py-14 px-6 rounded-2xl shadow-xl max-w-3xl">
          <h2 className="text-3xl font-extrabold mb-3">
            Know Your Legal Rights
          </h2>
          <p className="text-base md:text-lg text-gray-200 mb-6">
            Get expert criminal law updates, case analyses, and legal insights delivered to your inbox.
          </p>
          <a
            href="/newsletter"
            className="inline-block bg-white text-red-700 font-semibold px-6 py-3 rounded-md hover:bg-gray-100 transition-all duration-300"
          >
        ...
        {loading ? (
          <p className="text-center py-10">Loading criminal law posts...</p>
        ) : posts.length === 0 ? (
          <p className="text-gray-500 text-center">
            No criminal law posts found. Check back soon for updates.
          </p>
        ) : (
