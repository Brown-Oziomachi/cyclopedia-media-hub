 ...
 function DevelopersPage() {
  ...
  return (
    <main className="bg-black min-h-screen text-white z-0">
      ...
      <div className="absolute inset-0 items-center justify-center opacity-30 z-0 shadow-2xl shadow-black ">
        <img
          src="web1.jpg"
          alt="image"
          className="py-15 w-full mx-auto z-0 shadow-2xl shadow-black "
        />
      </div>
      <header className="flex flex-col justify-center items-center h-screen text-center px-5 relative z-0">
        <div className="space-y-6 max-w-xl -mt-20">
          <h1 className="text-5xl font-extrabold tracking-tight text-gray-300 z-0">
          ...
          <Link href="/contact">
            <button className="shadow-2xl shadow-black  z-50 border-y px-8 py-3 border-y-green-600 rounded-md font-semibold hover:bg-white hover:text-black transition">
              Get Started
            </button>
          </Link>
        </div>
      ...
      <section className="py-1 border-t border-green-600 max-md:-mt-35">
        <div className="max-w-6xl mx-auto px-5">
          ...
          <img
            src="web14.jpg"
            alt="image"
            className="mx-auto min-md:hidden shadow-2xl shadow-black rounded-bl-4xl"
          />
          <h1 className="text-sm text-center border-b border-x mb-5 text-gray-400 lg:hidden border-x-green-600 border-b-green-600">
            contact us to build your own system
          </h1>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 shadow-2xl shadow-black ">
            <div className="p-6 border border-green-600 rounded-lg hover:border-white transition shadow-2xl shadow-black ">
        ...
        <div className="max-w-7xl mx-auto px-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 ">
            {developersData.map((dev, idx) => (
              <div
                key={idx}
                className="shadow-2xl shadow-black  border border-green-600 rounded-lg p-6 flex flex-col items-center text-center hover:border-white transition"
              >
                <img
                  src={dev.image}
                  alt={dev.name}
                  className="w-36 h-36 rounded-full object-cover mb-6 shadow-2xl shadow-black "
                  loading="lazy"
                />
                ...
                <div className="flex gap-4 flex-wrap justify-center">
                  {dev.links.map((link, i) => (
                    <Link
                      key={i}
                      href={link.href}
                      className="shadow-2xl shadow-black  border-r border-r-green-600 px-4 py-2 rounded hover:bg-white hover:text-black transition text-sm z-50"
                      target={
